    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1 relative z-10">Life-to-Date Impact</p>
        ...
        <div className="bg-emerald-950 rounded-[2rem] p-8 shadow-xl shadow-emerald-200 lg:col-span-2 flex flex-col justify-center overflow-hidden">
           <div className="flex items-center justify-between">
             <div className="relative z-10">
               <p className="text-emerald-400 text-[10px] font-black uppercase tracking-widest mb-1">Latest Industry Benchmark</p>
               <h3 className="text-2xl font-black text-white">{latestAudit?.industry || 'Awaiting Data'}</h3>
             </div>
             <div className="scale-75 origin-right">
               {latestAudit ? (
                 <BenchmarkGauge score={latestAudit.industryBenchmark} industry="" />
               ) : (
                 <div className="w-40 h-20 bg-emerald-900/50 rounded-t-full border-t border-emerald-800 flex items-center justify-center text-emerald-700 font-bold text-[10px] uppercase">No Peer Data</div>
               )}
             </div>
           </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-sm">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h3 className="text-xl font-black text-slate-800 tracking-tight leading-none mb-1">Consolidated Footprint Trends</h3>
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Advisor Insight: Efficient growth detected</p>
            </div>
          </div>
              ...
              <BarChart data={chartData}>
                ...
                <Bar dataKey="value" radius={[8, 8, 0, 0]}>
                  {chartData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={index === chartData.length - 1 ? '#064e3b' : '#10b981'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="bg-emerald-50/30 rounded-[2.5rem] p-8 border border-emerald-100 shadow-sm flex flex-col">
          <div className="flex items-center gap-2 mb-8">
            ...
            <div>
              <h3 className="text-lg font-black text-emerald-950 tracking-tight leading-none">Strategic Wins</h3>
              <p className="text-[10px] font-bold text-emerald-700/60 uppercase tracking-widest">High ROI Actions</p>
            </div>
          </div>
                ...
                <div key={i} className="group p-5 bg-white rounded-3xl border border-emerald-50 transition-all hover:shadow-xl hover:-translate-y-1 cursor-pointer">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-emerald-950 text-emerald-400 uppercase tracking-widest">
                      {win.financialSave} Saved
                    </span>
                  </div>
                  <h4 className="font-black text-slate-800 text-sm mb-1 leading-tight">{win.title}</h4>
                  <p className="text-[11px] text-slate-500 leading-relaxed">{win.description}</p>
                </div>
              ))
            ) : (
              <div className="text-center py-16 flex flex-col items-center">
                <p className="text-slate-400 text-xs font-bold leading-relaxed px-4 italic">
                  Run a Strategic Audit to activate peer benchmarking and solar analysis.
                </p>
              </div>
            )}
          </div>
